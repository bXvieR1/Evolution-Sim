...
class Agent():
    ...
    def raycast(self, foods, agents):
        ...
        f = self.position - foods.position
        norm_f = numpy.linalg.norm(f, axis=1)

        valid_food_indices = numpy.where(norm_f < 100)[0]

        c_values = numpy.dot(f[valid_food_indices], f[valid_food_indices].T) - r ** 2
        b_values = 2 * numpy.dot(f[valid_food_indices], d.T)
        discriminant_values = b_values ** 2 - 4 * a * c_values[:, numpy.newaxis]
        valid_ray_indices = numpy.where(discriminant_values >= 0)[1]
        t1_values = (-b_values[:, valid_ray_indices] - numpy.sqrt(discriminant_values[:, valid_ray_indices])) / (2 * a[valid_ray_indices])
        valid_t1_indices = (t1_values >= 0) & (t1_values <= ray[valid_ray_indices])
        type[valid_ray_indices[valid_t1_indices]] = 1
        pygame.draw.line(screen, (255, 0, 0), self.position, self.position + d[valid_ray_indices[valid_t1_indices]] * t1_values[valid_t1_indices], 1)
        return ray
